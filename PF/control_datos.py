...
def mostrar_Dsecc():
    ...
    try:
        ...
        if datos!=[]:
            ...
            if opc_excel=="1":
                try:
                    if os.path.isfile(f"PF\\Visualizador.xlsx"):
                        os.remove('PF\Visualizador.xlsx')
                        wb=Workbook()
                        wb.save('PF\Visualizador.xlsx')   
                    exc=openpyxl.load_workbook('PF\Visualizador.xlsx')
                    h_exc=exc.active 
                    c=[]
                    ...
                    for d in range(0,len(datos)):
                        ...
                        h_exc.append(c)
                        c=[]          
                    exc.save('PF\Visualizador.xlsx')
                    if os.path.isfile(f"PF\\Visualizador.xlsx"):
                        os.startfile("PF\\Visualizador.xlsx")
                    else:
                        pausa_borra('n') 
...
def añadir_Dsecc():
    ...
    for crt in range(1,len(caracteristicas)):
        ...
        try:
            temp_TVal=""
            print(str(caracteristicas[crt][1])[0:3])
            if str(caracteristicas[crt][1])[0:3]=="int":
                temp_TVal="Numerico sin punto decimal o coma"    
            elif str(caracteristicas[crt][1])[0:5]=="float":
                temp_TVal="Numerico"
            elif str(caracteristicas[crt][1])[0:5]=="date":
                temp_TVal="Fecha con formato: YYYY-MM-DD" 
            elif str(caracteristicas[crt][1])[0:5]=="date":
                temp_TVal="Fecha con formato: YYYY-MM-DD"
            else:
                temp_TVal="Segun se necesite"
                                          
            temp_v=input(f"Ingresa el dato ({temp_TVal}) del campo: {caracteristicas[crt][0]} ‣ ")
        except ValueError:
            pausa_borra('b')
                ...
                if str(e)[0:len(mensaje)]==mensaje:
                    for i in str(e)[len(mensaje)+2:len(str(e))]:
                        if i!="'":
                            ...
                            break
                    pausa_borra('b')
                    print(f"Ingresa los datos del campo segun sea para el valor: {t_camp}")
                                        ...
                                        if str(e)[0:len(mensaje)]==mensaje:
                                            for i in str(e)[len(mensaje)+2:len(str(e))]:
                                                if i!="'":
                                                    ...
                                                    break
                                            pausa_borra('b')
                                            print(f"Ingresa el valor correcto que se necesite en : {t_camp}")
                                            print(f"Motivo referido :{e}")
                                            print("Intentalo otra vez")
                                            pausa_borra('p')
                                            break 
