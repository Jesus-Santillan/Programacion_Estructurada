...
def cambiar_nombre():
    ...
    if os.path.isfile(f"PF\\secciones\\{nombre_nv}.txt"):
        os.system("cls")
        print("<El nombre espesificado ya esxiste>") 
    else:      
        try:   
            if(os.path.exists("PF\\secciones\\.txt")==True and os.path.exists(f"PF\\secciones\\{nombre_nv}.txt")==False):
                os.system("cls")
                print("Algo paso al momento de cambiar el nombre")
                print("NO fue posible cambiar el nombre")    
                os.rename(f"PF\\secciones\\{nombre_nv}.txt",p_archivo)   
                pausa_borra('p') 
            else:
                cursor = conn.cursor()
                ...
                archivo=open("PF\\datos_conFS.txt","r")
                os.rename(p_archivo,f"PF\\secciones\\{nombre_nv}.txt") 
                datos=archivo.readlines()
                archivo.close()
                archivo=open("PF\\datos_conFS.txt","w")
                for i in range(0,len(datos)):
                    if(i==(num_secc-1)):                
                        archivo.write(f"{nombre_nv}\n")    
                    elif(i==(num_secc+num_r)-1):
                        archivo.write(f"PF\\secciones\\{nombre_nv}.txt\n")   
                    else:
...
def agregar_secc(num_rl):
    ...
    if os.path.isfile(f"PF\\secciones\\{nombre_SecN}.txt"):
        pausa_borra('b')
        print("<El nombre espesificado ya esxiste>")
    elif nombre_SecN!="":            
        print("")
            ...
            try:
                ...
                if verif==False:   
                    ...
                    if(opc=="2"):   
                        ...
                        cursor.execute(sql)   
                        archivo=open(f"PF\\secciones\\{nombre_SecN}.txt", "x") 
                        archivo.close()
                        archivo_w=open(f"PF\\secciones\\{nombre_SecN}.txt", "w")
                        ...
                        archivo.write("")
                        archivo.close() 
                        archivo=open("PF\\datos_conFS.txt","a")
                        archivo.write(f"PF\\secciones\\{nombre_SecN}.txt \n")
                        archivo.close()  
                        conn.close() 
                        resultado=f"\n{'\033[0;92m'}{'\033[4;92m'}✅ La accion se realizo correctamente ✅ {'\033[0;0m'}"            
                    elif(opc=="1"):                       
...
def eliminar_Tdsecc():
    ...
    if(opc=="si"):
        try:
            cursor = conn.cursor()
            archivo=open("PF\\datos_conFS.txt")
            ...
            rmtree("PF\\secciones/") 
            os.remove("PF\\datos_conFS.txt")   
            os.mkdir("PF\\secciones/")
            archivo=open("PF\\datos_conFS.txt","a")
            archivo.write("----\n")
            archivo.close()
            conn.close()
        ...
        try:        
            ...
            match opc:
                ...
                case 2:
                    pausa_borra('b')
                    resultado=f"\n{'\033[4;93m'}{'\033[0;93m'} ⚠ La accion no pudo ser concretada ⚠ {'\033[0;0m'}"
                    archivo=open(f"PF\\secciones\\{secc_nomb}.txt","w")
